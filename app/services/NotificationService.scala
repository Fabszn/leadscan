...
class NotificationServiceImpl(db: Database, mailer: MailerClient, remote: RemoteClient) extends NotificationService with LoggerAudit {
  ...
  override def sendMail(dest: Seq[String], bodyText: Option[String], bodyHtml: Option[String]): Unit = {
    ...
    mailer.send(Email("Devoxx FR Sponsor - your Lead Generation System credentials", Settings.play.mailer.from, dest, bodyText, bodyHtml, bcc = Seq("fabrice@scala.io","nicolas.martignole@devoxx.fr","antonio.goncalves@devoxx.fr","horacio.gonzalez@gmail.com","zouheir.cadi@devoxx.fr")))
